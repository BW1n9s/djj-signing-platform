...
/**
 * 搜索最近 30 天内的送货相关邮件，按时间倒序，最多返回 10 封
 * Search delivery-related emails in the last 30 days, newest first, max 10
 */
async function searchDeliveryEmails() {
  const auth = getOAuth2Client();
  const gmail = google.gmail({ version: 'v1', auth });

  // 计算 30 天前的日期，格式 YYYY/MM/DD（Gmail query 格式）
  // Compute date 30 days ago in Gmail query format YYYY/MM/DD
  ...
  // 并发拉取每封邮件完整内容 / Fetch full content for each message concurrently
  ...
  // 转换为统一格式并按时间倒序排列
  // Normalise format and sort newest first
  return emails
    .map(email => {
 ...
 * Extract delivery data fields from email content (best-effort; missing fields stay empty)
 */
async function parseEmailToDeliveryData(email) {
  const data = {
    invoice_no: '',
    invoice_date: '',
    customer_name: '',
  ...
  // 各字段正则映射 / Regex map for each field
  const patterns = {
...
/**
 * 从 MIME payload 中递归提取纯文本正文（优先 text/plain，回退 text/html）
 * Recursively extract plain-text body from MIME payload (prefer text/plain, fall back to text/html)
 */
function extractBodyText(payload) {
  ...
  if (payload.mimeType === 'text/html' && payload.body?.data) {
    ...
    // 简单去除 HTML 标签和多余空白 / Strip HTML tags and collapse whitespace
  ...
  if (payload.parts?.length) {
    ...
    // 递归处理 multipart / Recurse into nested multipart
...
/**
 * 尝试从正文文本中解析货物列表
 * Try to parse a list of delivery items from body text
 * 匹配 "3 x Widget XL" 或 "Qty: 2  Description: Chair" 格式
 * Matches "3 x Widget XL" or "Qty: 2  Description: Chair" patterns
 */
...
module.exports = { searchDeliveryEmails, parseEmailToDeliveryData };
